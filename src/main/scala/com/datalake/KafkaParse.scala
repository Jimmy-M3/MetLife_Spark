...
import org.apache.spark.sql.functions.{col, concat, concat_ws, from_json}
...
object KafkaParse {
  def main(args: Array[String]): Unit = {
    ...
    val schema:StructType = new StructType()
      .add("ZACTCODE", StringType, true)
      .add("BILLFREQ", StringType, true)
    ...
    val df5 = df4.select(concat_ws(sep=" || ",df4.col("key"),df4.col("zactcode"),df4.col("billfreq"),df4.col("timestamp")).as("data"))

    df5.writeStream.outputMode("append").format("text").option("checkpointLocation", "hdfs://cnszqldlkap02.alico.corp:8020/tmp/checkpoint2").option("path", "hdfs://cnszqldlkap02.alico.corp:8020/warehouse/tablespace/external/hive/rt_sync_tmp.db/rdz_la_zpcmpf_test2").trigger(Trigger.ProcessingTime("2 seconds")).start().awaitTermination()
